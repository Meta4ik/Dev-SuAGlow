/**
 * SuA Glow Typography A/B Testing Widget (Montserrat vs. Instrument Sans)
 * Allows real-time switching between Montserrat and Instrument Sans heading fonts.
...
(function () {
    ...
    // Graceful migration if previously set to urbanist, cormorant, or jost
    if (savedFont === 'urbanist' || savedFont === 'cormorant' || savedFont === 'jost') {
        savedFont = 'instrument';
        localStorage.setItem(STORAGE_KEY, 'instrument');
    }

    if (savedFont === 'instrument') {
        document.documentElement.classList.add('font-instrument');
    } else {
        document.documentElement.classList.remove('font-instrument');
    }

    // Ensure Google Font preconnect and Instrument Sans font stylesheet are loaded
    function ensureFontLoaded() {
        if (!document.getElementById('instrument-font-link')) {
            ...
            fontLink.id = 'instrument-font-link';
    ...
    function initWidget() {
        ...
        const currentFont = localStorage.getItem(STORAGE_KEY) === 'instrument' ? 'instrument' : 'montserrat';
        ...
        widget.innerHTML = `
            ...
            <div id="font-ab-card" class="bg-[#121518]/95 backdrop-blur-xl border border-warm-gold/45 rounded-full p-1.5 sm:p-2 shadow-[0_12px_35px_rgba(0,0,0,0.65),0_0_25px_rgba(170,152,124,0.25)] flex items-center gap-2 transition-all duration-300">
                ...
                <div class="flex items-center gap-1.5 pl-2.5 pr-1 py-1">
                    <span class="w-2 h-2 rounded-full ${currentFont === 'instrument' ? 'bg-[#AA987C]' : 'bg-white/40'} animate-pulse"></span>
                ...
                <div class="flex items-center bg-black/40 rounded-full p-1 border border-white/10">
                    ...
                    <button id="btn-font-instrument" type="button" 
                        class="px-3 py-1.5 rounded-full text-[10px] sm:text-[11px] font-semibold tracking-wider uppercase transition-all duration-200 whitespace-nowrap ${currentFont === 'instrument' ? 'bg-warm-gold text-near-black shadow-md font-bold' : 'text-white/60 hover:text-white'}">
                        Instrument Sans
                    </button>
                </div>
            ...
            <button id="btn-font-minimized-trigger" type="button" title="Open Font A/B Test (Press Alt+F to toggle)" aria-label="Open Font A/B Test"
                ...
                <span class="text-[7px] font-bold tracking-widest uppercase text-warm-gold mt-0.5 leading-none" id="mini-font-indicator">${currentFont === 'instrument' ? 'INS' : 'MON'}</span>
            </button>
        ...
        const btnInst = document.getElementById('btn-font-instrument');
        ...
        function setFont(font) {
            if (font === 'instrument') {
                ...
                btnInst.className = 'px-3 py-1.5 rounded-full text-[10px] sm:text-[11px] font-semibold tracking-wider uppercase transition-all duration-200 whitespace-nowrap bg-warm-gold text-near-black shadow-md font-bold';
                ...
                miniIndicator.textContent = 'INS';
                showToast('Headings: Instrument Sans');
            } else {
                document.documentElement.classList.remove('font-instrument');
                ...
                btnInst.className = 'px-3 py-1.5 rounded-full text-[10px] sm:text-[11px] font-semibold tracking-wider uppercase transition-all duration-200 whitespace-nowrap text-white/60 hover:text-white';
        ...
        btnInst.addEventListener('click', () => setFont('instrument'));
        ...
        window.addEventListener('keydown', (e) => {
            if (e.altKey && (e.key === 'f' || e.key === 'F')) {
                e.preventDefault();
                const current = localStorage.getItem(STORAGE_KEY) === 'instrument' ? 'instrument' : 'montserrat';
                setFont(current === 'instrument' ? 'montserrat' : 'instrument');
            }
        });
    }
