...
// Download Live Webpage Flyer Card Element as Image
function downloadFlyerImage() {
    const flyerElement = document.getElementById('flyer-card-element');
    const downloadBtn = document.getElementById('download-flyer-btn');

    if (!flyerElement) return;

    if (downloadBtn) {
        downloadBtn.innerHTML = `<i data-lucide="loader-2" class="w-4 h-4 animate-spin"></i> Generating Image...`;
    ...
    if (typeof html2canvas === 'undefined') {
        // Dynamically load html2canvas if script tag hasn't finished loading yet
        const script = document.createElement('script');
        script.src = 'https://cdnjs.cloudflare.com/ajax/libs/html2canvas/1.4.1/html2canvas.min.js';
        script.onload = renderAndDownload;
